...
function applyAdminTheme(theme) {
  adminTheme = theme === 'light' ? 'light' : 'dark';
  document.documentElement.setAttribute('data-theme', adminTheme);
  try { localStorage.setItem('s33-theme', adminTheme); } catch (_) {}
}

try { applyAdminTheme(localStorage.getItem('s33-theme')); } catch (_) { applyAdminTheme('dark'); }
...
async function publishDataset({ candidate, resultBox, onDone, button }) {
  ...
  try {
    ...
    */
    ...
    // После публикации результатов сохраняем срез позиций: база сравнит его
    // с предыдущим и уведомит только подписчиков реально сдвинувшихся альянсов.
    if (changes.some((change) => change.entity === 'result') && typeof forum.recordAllianceRankSnapshot === 'function') {
      ...
      } catch { /* результаты опубликованы; снимок повторится при следующей публикации */ }
    }
...
document.addEventListener('click', async (e) => {
  ...
  */
  ...
  if (e.target.closest('[data-admin-theme]')) {
    applyAdminTheme(adminTheme === 'light' ? 'dark' : 'light');
    render();
    return;
  }

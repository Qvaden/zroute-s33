 ...
 *
 * Здесь записывается всё, что делал сервер целиком: брал чужие Столицы
 * и отбивал свою. Плюс войны, слияния альянсов и прочее.
 *
 * Номер сервера в форме значит разное: при захвате — чью Столицу берём,
 * при защите — кто на нас шёл. Своя Столица у нас одна, 33-го сервера,
  ...
  return `
    <section class="adm-hero adm-hero--tight">
      ...
      <p class="adm-lead">
        ...
        ${plural(captures.length, 'Столица взята', 'Столицы взято', 'Столиц взято')}
      </p>
    </section>
...
function empty() {
  return `<p class="muted adm-note">
    ...
    Добавьте первую: взятую Столицу, отбитое нападение, войну
  ...
  return `
    <section class="panel adm-form" data-event-form>
      ...
      <p class="adm-form__hint muted">
        ${
          meta.action === 'defense'
            ? 'Защищаем мы только свою Столицу 33 сервера, поэтому здесь номер того, кто нападал, а не свой.'
...
function titlePlaceholder(type) {
  return {
    server_capture: 'Взята Столица сервера 74',
    capture_failed: 'Не взяли Столицу сервера 52',
    server_defended: 'Отбили нападение сервера 51',
    server_lost: 'Столица потеряна: напал сервер 19',

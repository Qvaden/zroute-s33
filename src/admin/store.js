...
/**
 * Выдать или снять право править данные сайта.
 *
 * Идёт через функцию в базе, а не запросом к таблице профилей: правку профилей
 * разрешено модерации, а признак редактора — дело администратора. Функция
 * проверяет это сама, поэтому право «назначить редактора» отдаётся в панель
 * без права «менять что угодно в профилях».
 */
export async function setEditor(nick, allow) {
  await rest('/rpc/site_set_editor', {
    method: 'POST',

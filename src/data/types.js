 ...
 * @property {string}  [mergedInto] Id альянса, в который этот вошёл слиянием.
 *   Если заполнено, альянс обязан быть неактивным (active: false), но его
 *   история результатов остаётся закреплена за ним самим — очки поглотившего
 *   альянса задним числом не пересчитываются, это было бы переписью истории.
 */

/**
 * Игровая неделя (один цикл VS).
 *
 * У недели ровно одна обязанность: счёт альянсов в VS. Что делал сервер
 * целиком — захватил чужой, удержал свой — живёт в хронологии отдельными
 * событиями, у которых своя дата: кампания может тянуться через несколько
 * недель, а один и тот же факт не должен записываться в двух местах.
 *
 * @typedef {Object} Week

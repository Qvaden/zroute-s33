...
export async function currentUser() {
  ...
  const rows = await rest('/forum_users?select=*&limit=1');
  const me = Array.isArray(rows) ? rows[0] : null;
  /*
    Политика доступа отдаёт этому запросу только собственную строку, поэтому
    первая и есть я. Пустой ответ значит, что сессия ещё жива, а профиля
    уже нет — например, аккаунт удалили. Тогда сессия недействительна.
  */
  if (!me) {
    writeSession(null);
    return null;

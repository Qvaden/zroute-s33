...
/**
 * РЕАКЦИИ В ЧАТЕ — НЕ ТО ЖЕ, ЧТО НА ФОРУМЕ.
 *
 * В ленте реакция отвечает на вопрос «согласен или нет» и двигает очки
 * записи, поэтому их нельзя ставить две разом. В чате это отклик на реплику
 * в разговоре: тут ставят и «смешно», и «ого» на одну фразу, и никакого
 * счёта за этим нет.
 *
 * Набор намеренно короткий и без «не согласен»: спор в чате ведут словами,
 * а дизлайк без объяснения превращает разговор в перепалку. Список тот же,
 * что в подсказке быстрых реакций, и тот же проверяет база триггером
 * (supabase/chats.sql) — иначе реакцией можно было бы прислать что угодно.
 */
export const CHAT_REACTIONS = ['👍', '❤️', '🔥', '😂', '😮', '😢', '🫡', '👏'];

/** Эмодзи допустим как реакция чата. */
export function isChatReaction(emoji) {
  return CHAT_REACTIONS.includes(String(emoji));
}

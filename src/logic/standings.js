...
/**
 * Недели, по которым сайту есть что показывать: с первой и до последней,
 * за которую внесён хоть один результат. Хвост будущих недель отбрасывается.
 *
 * Зачем: в таблице недели заводятся заранее, на месяц-полтора вперёд — иначе
 * пришлось бы каждую неделю добавлять столбец руками. Но «итогами недели»
 * нельзя объявлять неделю, которая ещё не наступила: сайт показал бы номер
 * из будущего и пустой список победителей.
 *
 * Внутренние пропуски сохраняются намеренно. Если за какую-то неделю данных
 * нет, а за следующую есть — эта неделя остаётся в наборе, и на графике будет
 * видно горизонтальный участок. Дырка в данных должна быть заметна, а не
 * заметена под ковёр сжатием шкалы.
 *
...
export function weeksUpToLastData(weeks, results) {
  ...
  let last = -1;
  ordered.forEach((w, i) => {
    if (filled.has(w.id)) last = i;
  });

  return last < 0 ? [] : ordered.slice(0, last + 1);
}

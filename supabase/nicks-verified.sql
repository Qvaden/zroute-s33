...
--
-- Иммутабельная функция для индекса. Нормализует нижний регистр, full-width
-- буквы и лигатуры (NFKC), затем подменяет «двойники»:
--   • кириллические «лatinские» буквы (Крeмль == Кремль);
--   • украинские і/ї (== i), є (== e);
--   • греческие ο/ι/ε/ρ/τ/χ/κ/ν (== o/i/e/p/t/x/k/v);
--   • латинские с диакритикой (é/à/ä/etc → e/a/a);
--   • цифры и похожие буквы: 0 == O, 1 == l == I == i.
-- Разделители (пробел, _, -) не трогаем: «Ковыль» и «Ковыль-33» —
-- разные люди, их нельзя склеивать.
-- Требуется PostgreSQL 13+ (normalize).

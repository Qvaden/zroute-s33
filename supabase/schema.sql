...
--
-- Здесь была функция forum_admin_reset_password: панель придумывала пароль и
-- просила базу записать его. От неё отказались — см.
-- supabase/20260925-self-recovery.sql. Игрок восстанавливает доступ сам и
-- вводит пароль в своё поле, поэтому знать его должен один человек, а не двое.
-- Функцию ниже этот же скрипт удаляет из уже развёрнутых баз.

-- ── УДАЛЕНИЕ АККАУНТА ────────────────────────────────────────────────────────
--
-- Той же схемой, что и другие административные функции: функцию в базу, а не
-- запрос из панели.
